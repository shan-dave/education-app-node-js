...
const queansstudentdashbord_model_1 = tslib_1.__importDefault(require("../models/queansstudentdashbord.model"));
...
class QueAnsStudentDeashbordService {
    constructor() {
        this.queAnsStudentDeashbord = queansstudentdashbord_model_1.default;
        this.students = students_model_1.default;
    }
    async createQueAnsStudentDashbord(queAnsData, userData) {
        const todayDate = moment_1.default().utc().format("YYYY-MM-DD");
        const todayReportExist = await this.queAnsStudentDeashbord.findOne({
            createdAt: { $gte: moment_1.default.utc(todayDate).set({ hours: 0, minutes: 0, seconds: 0, milliseconds: 0 }).toDate() },
            studentId: queAnsData.studentId,
            teacherId: userData._id
        });
        let studentRecords = await this.students.findOne({ _id: queAnsData.studentId, isDeleted: false });
        if (!studentRecords)
            throw new HttpException_1.default(409, `Student not found`);
        else if (studentRecords.isActivated == true)
            throw new HttpException_1.default(409, `Student is Deactivated`);
        else if (studentRecords.schoolId.toString() != userData.schoolId.toString())
            throw new HttpException_1.default(409, `Student is not belongs to your school`);
        if (todayReportExist)
            throw new HttpException_1.default(409, `report already submit for this student`);
        const createQueAnsStudentDashbordData = await this.queAnsStudentDeashbord.create(Object.assign(Object.assign({}, queAnsData), { teacherId: userData._id, schoolId: userData.schoolId, districtId: queAnsData.districtId }));
    ...
    //   return getQueData
    // }
    async getQueAnsStudentDashbordListChart1(query, user) {
        let search = {
            $and: [{ isDeleted: false }]
        };
        if (query.startDate && query.endDate) {
            search.$and.push({ createdAt: { $gte: moment_1.default.utc(query.startDate).set({ hours: 0, minutes: 0, seconds: 0, milliseconds: 0 }).toDate() } });
            search.$and.push({ createdAt: { $lte: moment_1.default.utc(query.endDate).set({ hours: 23, minutes: 59, seconds: 59, milliseconds: 999 }).toDate() } });
        }
        else {
            throw new HttpException_1.default(409, `Please enter start and end date`);
        }
        if (user.role.name == "Super-Admin") {
            if (query.districtId) {
                search.$and.push({
                    districtId: new mongoose_1.default.Types.ObjectId(query.districtId)
                });
            }
            if (query.schoolId) {
                search.$and.push({
                    schoolId: new mongoose_1.default.Types.ObjectId(query.schoolId)
                });
            }
            if (query.teacherId) {
                search.$and.push({
                    teacherId: new mongoose_1.default.Types.ObjectId(query.teacherId)
                });
            }
            if (query.studentId) {
                search.$and.push({
                    studentId: new mongoose_1.default.Types.ObjectId(query.studentId)
                });
            }
        }
        else if (user.role.name == "School-Admin" || user.role.name == "School-Sub-Admin") {
            ...
            if (user.schoolId) {
                search.$and.push({
                    schoolId: new mongoose_1.default.Types.ObjectId(user.schoolId)
                });
            }
            if (query.teacherId) {
                search.$and.push({
            ...
            if (user._id) {
                search.$and.push({
                    teacherId: new mongoose_1.default.Types.ObjectId(user._id)
                });
            }
            if (query.studentId) {
                search.$and.push({
        ...
        if (query.questionId) {
            search.$and.push({
                "queAns.questionId": new mongoose_1.default.Types.ObjectId(query.questionId)
            });
        }
        search.$and.push({
            "queAns.questionId": new mongoose_1.default.Types.ObjectId('61604ae6e24b797dc90908e3')
        });
        if (query.answer) {
            search.$and.push({
                "queAns.answer": query.answer
        ...
        if (loadsh_1.default.isEmpty(sortBy)) {
            sortBy = { schoolName: 1 };
        }
        const getQueData = await this.queAnsStudentDeashbord.aggregate([
            {
                $unwind: {
                    path: '$queAns',
    ...
    async getQueAnsStudentDashbordListAttendanceChart(query, user) {
        let search = {
            $and: [{ isDeleted: false }]
        };
        if (query.startDate && query.endDate) {
        ...
        search.$and.push({
            "queAns.questionId": new mongoose_1.default.Types.ObjectId('616049dbe24b797dc9090890')
        });
        if (query.answer) {
            search.$and.push({
                "queAns.answer": query.answer
    ...
    async getQueAnsStudentDashbordListChart2(query, user) {
        let search = {
            $and: [{ isDeleted: false }]
        };
        if (query.startDate && query.endDate) {
    ...
    async getQueAnsStudentDashbordListChart3(query, user) {
        let search = {
            $and: [{ isDeleted: false }]
        };
        if (query.startDate && query.endDate) {
        ...
        search.$and.push({
            "queAns.questionId": new mongoose_1.default.Types.ObjectId('61604a2ee24b797dc90908a4')
        });
        if (query.answer) {
            search.$and.push({
                "queAns.answer": query.answer
    ...
    async getQueAnsStudentDashbordListChart4(query, user) {
        let search = {
            $and: [{ isDeleted: false }]
        };
        if (query.startDate && query.endDate) {
        ...
        search.$and.push({
            "queAns.questionId": new mongoose_1.default.Types.ObjectId('61604ac9e24b797dc90908d8')
        });
        if (query.answer) {
            search.$and.push({
                "queAns.answer": query.answer
    ...
    async getQueAnsStudentDashbordListChart5(query, user) {
        let search = {
            $and: [{ isDeleted: false }]
        };
        if (query.startDate && query.endDate) {
        ...
        search.$and.push({
            "queAns.questionId": new mongoose_1.default.Types.ObjectId('61604afbe24b797dc90908ee')
        });
        if (query.answer) {
            search.$and.push({
                "queAns.answer": query.answer
    ...
    async getQueAnsStudentDashbordListChart6(query, user) {
        let search = {
            $and: [{ isDeleted: false }]
        };
        if (query.startDate && query.endDate) {
        ...
        search.$and.push({
            "queAns.questionId": new mongoose_1.default.Types.ObjectId('61604b11e24b797dc90908f9')
        });
        if (query.answer) {
            search.$and.push({
                "queAns.answer": query.answer
...
exports.default = QueAnsStudentDeashbordService;
//# sourceMappingURL=queansstudentdashbord.service.js.map